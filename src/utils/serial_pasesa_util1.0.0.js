    ...
    constructor() {
        ...
        if (!SingletonPasesaSerial.instance) {
            ...
            this.startMeasure = async function (age, sex) {
                ...
                } else if (currentState == CONNECTSTATE.CONNECTING) {
                    callError(ERRORSTATE.ISCONNECTING)
                } else if (currentState == CONNECTSTATE.IDLE) {
                    //如果是闲置，就调用测量命令
                    start()
                } else if (currentState == CONNECTSTATE.WORK || currentState == CONNECTSTATE.STOPWORKING) {
            ...
            this.stopMeasure = function () {
                if (currentState == CONNECTSTATE.WORK) {
                    ...
                    STOPMEASURE.push(mAge)
                    STOPMEASURE.push(mSex)
            ...
            this.disconnectSerial = async function () {
                //只有连接了才能断开连接
                if (currentState == CONNECTSTATE.IDLE || currentState == CONNECTSTATE.WORK || currentState == CONNECTSTATE.STOPWORKING) {
                    //关闭串口时候，如果正在测量，就停止测量
                    if (currentState == CONNECTSTATE.WORK) {
                        this.stopMeasure()
                    }
